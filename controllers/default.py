...
class RemotePicture(RequestHandler):
    ...
    def upload(self,*args, **kwargs):
        ...
        # 解析JSON格式的配置文件
        with open(os.path.join(static_path, 'ueditor', 'config.json')) as fp:
            try:
                # 删除 `/**/` 之间的注释
        ...
        if action == 'config':
            ...
            if fieldName in self.request.files:
                ...
                for fieldsss in field:
                    uploader = ueditor.Uploader(fieldsss, config, static_path)
            ...
            if fieldName in self.request.form:
                ...
                uploader = ueditor.Uploader(field, config, static_path, 'base64')
                result = uploader.getFileInfo()
            else:
                result['state'] = '上传接口出错'
            ...
            for imgurl in source:
                uploader = ueditor.Uploader(imgurl, config, static_path, 'remote')
